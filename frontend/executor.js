...
export class SessionExecutor {
     ...
     */
    constructor(device, sessionGraph, uiManager, safetensorCache = null, profiler = null, tracing = false, outputCache = null) {
        ...
        this.outputCache = outputCache || new OutputCache();
    ...
    async _executeGPUSession(session) {
        ...
        for (const [nodeIndexInSession, node] of session.nodes.entries()) {
            ...
            /* Check output cache */
            let cached = false;
            if(node.cacheable && this.outputCache) {
                let all_nodes = true;
                for(const output of node.get_outputs()) {
                    const outputKey = `${node.name}:${output}`;
                    const cachedCPU = this.outputCache.getCPU(outputKey);
                    if(cachedCPU) {
                        this.cpuOutputs.set(outputKey, cachedCPU);
                    }
                    const cachedGPU = this.outputCache.getGPU(outputKey);
                    if(cachedGPU) {
                        this.gpuOutputs.set(outputKey, cachedGPU);
                    }

                    if(!cachedCPU && !cachedGPU) {
                        all_nodes = false;
                        break;
                    }
                }

                // If we have all the outputs cached, no need to execute the node!
                if(all_nodes) {
                    if(this.profiler) {
                        this.profiler.outputCacheHits++;
                    }
                    cached = true;
                } else {
                    if(this.profiler) {
                        this.profiler.outputCacheMisses++;
                    }
                }
            }
            ...
            try {
                if(!cached) {
                    ...
                    for (const output of kernel.outputs) {
                        ...
                        // Put this output in the output cache
                        if(node.cacheable) {
                            this.outputCache.put(outputKey, outputTensor);
                        }
        ...
        for (const op of readbackOperations) {
            ...
            try {
                ...
                // Put this output in the output cache
                if(this.outputCache) {
                    this.outputCache.put(op.outputKey, cpuTensor);
                }
        ...
        for (const [nodeIndexInSession, node] of session.nodes.entries()) {
            ...
            if(node.cacheable && this.outputCache) {
                ...
                for(const output of node.get_outputs()) {
                    ...
                    if(!cachedCPU && !cachedGPU) {
                        console.log(`Node ${node.name} not cached because ${outputKey} is not cached`);
                        all_nodes = false;
                        break;
                    }
                }
