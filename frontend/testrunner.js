...
async function main() {
  try {
    ...
    const safetensorCache = new SafeTensorCache();

    while(true) {
      let work = await coordinator.get_work("");
      if (!work) {
      ...
      document.getElementById('current-partition').textContent = work.partition;
      ...
      if (sessionsToDisplay.length > 0) {
          sessionsToDisplay.forEach(session => {
              ...
              sessionTitle.textContent = `Session ID: ${session.index || 'Unknown Session'}`;
      ...
      const executor = new SessionExecutor(device, sessionGraph, safetensorCache);
      ...
      const work_response = await coordinator.submit_work(new PartitionWorkResult({
        partition: work.partition,
        correlation_id: work.correlation_id,
        outputs: o 
      }))
      // const work_response_json = await work_response.json();
      // if (!work_response_json) {
      //   break;
      // }
    }
  } catch (error) {
