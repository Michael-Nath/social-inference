...
def llama_attn(
    ...
    hidden_shape = b.cat("bsz+seq_len+nheads", bsz_cat_seqlen, nhead_node, zero_node)
    ...
    with NameScope.push_scope("query_states"):
        q_weight = b.debug("q_weight_debug", q_weight)
        q_proj = b.matmul("q_proj", hidden_states, q_weight) # (bsz, seq_len, hidden_dim)
        q_proj = b.reshape("reshaped", q_proj, hidden_shape) # (bsz, seq_len, nheads, head_dim)
        query_states = b.transpose("transposed<1,2>", q_proj, 1, 2) # (bsz, nheads, seq_Len, head_dim)
        query_states = b.debug("debug", query_states)

    with NameScope.push_scope("key_states"):
        k_proj = b.matmul("k_proj", hidden_states, k_weight) # (bsz, seq_len, hidden_dim)
        k_proj = b.reshape("reshaped", k_proj, hidden_shape) # (bsz, seq_len, nheads, head_dim)
        key_states = b.transpose("transposed<1,2>", k_proj, 1, 2) # (bsz, nheads, seq_len, head_dim)
