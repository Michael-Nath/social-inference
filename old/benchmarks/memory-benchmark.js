...
async function memCheck() {
	...
	for (let power = startPower; power <= maxPower; power++) {
	    ...
	    const nBuffers = Math.ceil(size / maxBindedBufferSize);
		...
		try {
			...
			const bufferSize = Math.min(size - j * maxBindedBufferSize, maxBindedBufferSize);
			...
			for (let i = 0; i < nBuffers; i++) {
			    ...
			    for (let j = 0; j < Math.ceil(buffer.size / fillSize); j++) {
				const transferSize = Math.min(buffer.size - j * fillSize, fillSize);
				device.queue.writeBuffer(buffer, j * fillSize, dataArray, 0, transferSize);
			    }
			}
